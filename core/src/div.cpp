...
void Div::draw(sf::RenderWindow &win) {
    if (m_color.a > 0) {
        sf::RectangleShape bg_rect = sf::RectangleShape(m_size);
        bg_rect.setFillColor(m_color);
        bg_rect.setPosition(m_position);
        win.draw(bg_rect);
    }
    for (const auto elem : m_elements) {
        sf::Vector2f max_point = elem->get_position() + elem->get_size();
        if (max_point.x > m_size.x || max_point.y > m_size.y) {
            std::println("Error: Element too big!: curr/max x: {}/{}, curr/max y: {}/{}", max_point.x, m_size.x, max_point.y, m_size.y);
            continue;
        }
        elem->draw(win);
    }
...
void Div::add_element(Element *elem) {
    ...
    if (m_elem_ptr.x + elem_size.x < m_size.x) {
...
void Div::move_ptr_x(float x) {
    ...
    if (m_elem_ptr.x > m_size.x) m_elem_ptr.x = m_size.x;
}

void Div::move_ptr_y(float y) {
    this->m_elem_ptr.y += y;
    if (m_elem_ptr.y < 0) m_elem_ptr.y = 0;
    if (m_elem_ptr.y > m_size.y) m_elem_ptr.y = m_size.y;
}
